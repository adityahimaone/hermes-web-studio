...
- [x] Persist profile registry and active profile; migrate legacy unowned Spaces to `default` (malformed persisted state/migration failure fail closed with HTTP 503; rollback and profile-scoped preference/Space regressions verified)
...
- [~] P033 Complete Kanban boards/tasks/assignees/links/bulk actions/dispatch/events/stats and map its persisted state without local fallback (native CLI-backed board/read/create/dispatch/action slice plus safe edit/link actions, Chat-like Kanban rail, scroll-safe reference task modal, Spaces-tab registry binding, parent/runtime/skills fields, task detail drawer, preview, desktop drag targets, and rendered browser acceptance implemented; Dashboard upgrade, full detail editing, bulk, events, stats parity, migration, remote execution proof, and authenticated Dashboard proof remain open)
