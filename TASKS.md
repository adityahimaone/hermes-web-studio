...
- [~] P034 Complete Skills grouping/search/content/linked files/create/edit/delete/toggle/usage and profile-aware refresh (preview loading/empty/error state separation and shared collapsible ContextRail implemented; CRUD, linked files, usage, and profile refresh remain)
...
- [~] P067 Promote the shared ContextRail and seam collapse control to the full Chat, Skills, Memory, Profiles, and Settings layout shell (shared header/body zones, seam control, responsive widths, and one body scroll owner are now implemented; exact cross-page visual evidence remains)
- [~] P068 Implement the reference row variants inside the shared shell: compact Chat session rows, Skill toggle rows, Settings/Memory navigation rows, Profile status rows, and the Profiles feature-card/info slot (Chat and Profiles density are implemented; remaining cross-page row and visual matrix work remains)
- [~] P069 Apply the audited page grid and density contract: responsive 64px icon rail, 52–64px navbar, 280–320px sidebar, independent sidebar/content scrolling, seam control, and compact 40–48px rows (the responsive interpretation in ADR-047 is implemented; browser geometry evidence remains)
- [~] P070 Apply the literal type/color/component tokens across navbar, rail, sidebar, empty states, settings cards, chat transcript, and composer (semantic tokens, compact utility typography, 960px transcript, and 1040px composer widths are implemented; exact page-wide adoption remains)
- [~] P071 Complete navbar parity: profile/app/title ordering, optional count badge, right-side state controls, non-clipping password action, and page-specific single-line title behavior (password clipping and non-chat repeated subtitle fixed; composer context-window popover, compact selects, and overlay stacking added; bounded shell checks cover title/sidebar state, while full reference ordering and state matrix remain)
