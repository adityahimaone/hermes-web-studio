...
- [~] P027 Replace the single-root workspace abstraction with reference-compatible registered Spaces, active inheritance, ordering, health, suggestions, and profile-local state (server-owned registry now validates configured profile ownership, rejects unsafe refs, redacts remote refs, uses numeric ordering, and serializes activate/delete; UI validates normalized entries, supports delete/error states, and uses touch-safe controls; profile-local activation, inheritance, suggestions, update UI, and browser evidence remain)
