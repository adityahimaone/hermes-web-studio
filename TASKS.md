...
- [ ] P059 Run live Hermes/Gateway side-by-side rows for chat, tools, subagents, approvals, clarification, compression, recovery, external channels, and scheduled work (local acceptance runner added; 2026-08-31 health observed with Gateway unreachable, so live rows remain unclaimed)
...
- [~] P067 Promote the shared ContextRail and seam collapse control to the full Chat, Skills, Memory, Profiles, and Settings layout shell (Profiles now uses the shared rail and page seam toggle; exact reference geometry and cross-page visual evidence remain)
- [~] P068 Implement the reference row variants inside the shared shell: compact Chat session rows, Skill toggle rows, Settings/Memory navigation rows, Profile status rows, and the Profiles feature-card/info slot (Profiles now has a compact status row, active badge, feature slot, detail pane, and mobile fallback; the remaining cross-page row and visual matrix work remains)
- [~] P069 Apply the audited page grid and density contract: 88px icon rail, 64px navbar, 552px sidebar, independent sidebar/content scrolling, 44px seam control, and compact 40–48px rows (bounded shell checks now assert the current transitional geometry and responsive visibility; literal 88px/64px/552px reference thresholds remain)
