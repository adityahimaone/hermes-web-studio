...
- [~] P015 Preserve inflight turns across session switches, hard reload, reconnect backoff, cursor replay, and full-session poll fallback (journal restore, transcript restore, persisted cursor reconnect, stream/session guarded abortable pump/fallback, timeout without invented messages, EventSource replacement guards, and identity-preserving restored replies implemented; live proof remains)
