...
- [~] P023 Complete searchable provider-grouped model selection, live custom endpoint discovery, auxiliary models, aliases, quota/cost/TPS, reasoning, and toolsets (server-owned `/api/models/catalog` now probes Gateway `/v1/models`, returns explicit unavailable state without fallback models, and composer loads/searches provider-grouped models; auxiliary/runtime metadata, live custom endpoints, and browser/live proof remain)
