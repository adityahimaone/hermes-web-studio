...
- [~] P023 Complete searchable provider-grouped model selection, live custom endpoint discovery, auxiliary models, aliases, quota/cost/TPS, reasoning, and toolsets (server-owned `/api/models/catalog` validates local Gateway URLs, disables redirects, bounds/normalizes/deduplicates catalog values, enforces explicit body/item limits, returns explicit unavailable state without fallback models, and composer loads/searches semantic provider groups with explicit loading/ready/unavailable/error/empty states and fail-closed stale-profile blocking; auxiliary/runtime metadata, live custom endpoints, and browser/live proof remain)
