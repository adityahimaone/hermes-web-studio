...
- [~] P015 Preserve inflight turns across session switches, hard reload, reconnect backoff, cursor replay, and full-session poll fallback (journal restore, transcript restore, cursor reconnect, stale-stream protection, and expired-stream session polling fallback implemented; live proof remains)
