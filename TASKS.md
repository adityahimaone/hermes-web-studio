...
- [~] P015 Preserve inflight turns across session switches, hard reload, reconnect backoff, cursor replay, and full-session poll fallback (journal restore, transcript restore, cursor reconnect, stream/session guarded abortable fallback with timeout cleanup, and duplicate-safe replies implemented; live proof remains)
