...
try {
  for (const viewport of viewports) {
    ...
    if (viewport.width >= 1024) await expect(page.getByRole('complementary', { name: /(Recent sessions|Context Chat navigation)/ })).toBeVisible()
    else await expect(page.getByRole('complementary', { name: /(Recent sessions|Context Chat navigation)/ })).toBeHidden()
