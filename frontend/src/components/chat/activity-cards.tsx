...
export function ActivityCards({ tools, subagents, approvals }: { tools: ToolActivity[]; subagents: SubagentActivity[]; approvals: ApprovalRequest[] }) {
  ...
  return <div className="mb-4 space-y-2" aria-label="Hermes activity">
    ...
    {approvals.map((approval) => <div key={approval.id} className="approval-card"><ShieldAlert size={17} className="shrink-0 text-amber-300" /><div className="min-w-0 flex-1"><p className="font-medium">Approval required: {approval.name}</p>{approval.command && <code className="mt-1 block whitespace-pre-wrap break-words text-xs text-muted-foreground">{approval.command}</code>}{approval.reason && <p className="mt-1 text-xs text-muted-foreground">{approval.reason}</p>}</div>{approval.status !== 'pending' && <span className="text-xs capitalize text-muted-foreground">{approval.status}</span>}{approval.status === 'pending' && <div className="flex shrink-0 gap-1"><Button type="button" size="sm" variant="outline" disabled aria-label="Approve action (not connected)">Approve</Button><Button type="button" size="sm" variant="ghost" disabled aria-label="Deny action (not connected)">Deny</Button></div>}</div>)}
  </div>
}
