...
export function Composer({ onSend, onCancel, isStreaming, draft, onDraftChange, queuedMessages }: { onSend: (value: string) => void; onCancel: () => void; isStreaming: boolean; draft?: string; onDraftChange?: (value: string) => void; queuedMessages?: string[] }) {
    ...
    <div className="mx-auto w-full max-w-3xl px-3 pb-4 sm:px-6">
      <div className="rounded-2xl border bg-card/90 p-2 shadow-2xl shadow-black/30 backdrop-blur-xl focus-within:border-primary/35 focus-within:ring-1 focus-within:ring-primary/20">
        ...
        <div className="flex items-center gap-1 px-1 pb-1">
          <label className="inline-flex size-9 cursor-pointer items-center justify-center rounded-lg text-muted-foreground hover:bg-accent hover:text-accent-foreground focus-within:ring-2 focus-within:ring-ring" aria-label="Attach files"><Paperclip size={16} /><input type="file" multiple className="sr-only" onChange={chooseFiles} disabled={isStreaming} /></label>
      ...
      <p className="mt-2 text-center text-[10px] text-muted-foreground">{queuedMessages?.length ? `${queuedMessages.length} message${queuedMessages.length === 1 ? '' : 's'} queued` : attachments.length ? 'Files selected locally; multimodal sending is not connected yet.' : 'Hermes can make mistakes and run tools. Review important actions.'}</p>
    </div>
  )
}
