...
export function Sidebar({ onNavigate, currentView, mobileOpen, onClose, customizeOpen, onCustomizeOpenChange }: Props) {
  ...
  return <aside data-testid="primary-rail" className={cn('primary-rail h-screen w-16 shrink-0 flex-col items-center border-r bg-card/60 px-2 py-3 backdrop-blur-xl', mobileOpen ? 'fixed inset-y-0 left-0 z-50 flex w-[264px] items-stretch px-3 shadow-2xl lg:hidden' : 'hidden lg:flex')}>
    <div className={cn('flex h-11 items-center', mobileOpen ? 'gap-3 px-2' : 'justify-center')}><div className="grid size-9 shrink-0 place-items-center rounded-xl bg-gradient-to-tr from-primary/80 to-primary text-primary-foreground shadow-[0_0_20px_rgb(157_125_253/0.3)]"><Sparkles size={18} /></div><div className={cn('min-w-0', mobileOpen ? 'block' : 'hidden')}><div className="truncate text-sm font-semibold tracking-tight">Hermes Studio</div><div className="text-[10px] text-muted-foreground">Agent workspace</div></div>{mobileOpen && onClose && <Button ref={closeButton} variant="ghost" size="icon" className="ml-auto lg:hidden" onClick={onClose} aria-label="Close navigation"><X size={16} /></Button>}</div>
    <nav data-testid="primary-navigation" className={cn('mt-4 flex min-h-0 flex-1 flex-col gap-1', mobileOpen ? 'w-full' : 'w-12')} aria-label="Main navigation">{items.filter(item => visible.has(item.label)).map(({ label, icon: Icon }) => <Tooltip key={label} label={label}><Button variant="ghost" onClick={() => { onNavigate(label.toLowerCase()); onClose?.() }} aria-label={label} aria-current={currentView === label.toLowerCase() ? 'page' : undefined} className={cn('h-10 rounded-xl transition-all', mobileOpen ? 'w-full justify-start gap-2.5 px-3 text-xs' : 'w-11 justify-center px-0', currentView === label.toLowerCase() ? 'bg-primary/15 text-primary shadow-sm ring-1 ring-primary/20' : 'text-muted-foreground hover:bg-accent/60 hover:text-foreground')}><Icon aria-hidden="true" size={19} /><span className={cn(mobileOpen ? 'inline font-medium' : 'hidden')}>{label}</span></Button></Tooltip>)}</nav>
    <div className="mt-auto flex flex-col gap-1">
      <Tooltip label="Customize navigation"><Button variant="ghost" size="icon" className={cn('size-10 shrink-0 rounded-xl text-muted-foreground hover:bg-accent/60 hover:text-foreground', mobileOpen ? 'w-full justify-start px-3' : '')} onClick={() => onCustomizeOpenChange(true)} aria-label="Customize navigation"><SlidersHorizontal size={18} /><span className={cn(mobileOpen ? 'inline' : 'hidden')}>Customize navigation</span></Button></Tooltip>
      <Tooltip label="Settings"><Button variant="ghost" size="icon" className={cn('size-10 shrink-0 rounded-xl text-muted-foreground hover:bg-accent/60 hover:text-foreground', mobileOpen ? 'w-full justify-start px-3' : '')} onClick={() => { onNavigate('settings'); onClose?.() }} aria-label="Settings"><Settings2 size={19} /><span className={cn(mobileOpen ? 'inline' : 'hidden')}>Settings</span></Button></Tooltip>
    </div>
