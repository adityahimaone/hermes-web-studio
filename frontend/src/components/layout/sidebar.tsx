...
export function Sidebar({ onNavigate, currentView, mobileOpen, onClose, customizeOpen, onCustomizeOpenChange }: Props) {
  ...
  return <aside data-testid="primary-rail" className={cn('h-screen w-[72px] shrink-0 flex-col items-center border-r bg-card/55 px-2 py-3 backdrop-blur-xl', mobileOpen ? 'fixed inset-y-0 left-0 z-50 flex w-[264px] items-stretch px-3 shadow-2xl lg:hidden' : 'hidden lg:flex')}>
    ...
    <Tooltip label="Settings"><Button variant="ghost" size="icon" className={cn('mt-auto', mobileOpen ? 'hidden' : 'size-11')} onClick={() => { onNavigate('settings'); onClose?.() }} aria-label="Settings"><Settings2 size={19} /></Button></Tooltip>
