...
export function Sidebar({ onNavigate, currentView, mobileOpen, onClose, customizeOpen, onCustomizeOpenChange }: Props) {
  ...
  return <aside data-testid="primary-rail" className={cn('h-screen w-[88px] shrink-0 flex-col items-center border-r bg-card/55 px-3 py-3 backdrop-blur-xl', mobileOpen ? 'fixed inset-y-0 left-0 z-50 flex w-[264px] items-stretch px-3 shadow-2xl lg:hidden' : 'hidden lg:flex')}>
    <div className={cn('flex h-14 items-center', mobileOpen ? 'gap-3 px-2' : 'justify-center')}><div className="grid size-14 shrink-0 place-items-center rounded-2xl bg-primary text-primary-foreground shadow-[0_0_30px_rgb(139_92_246/0.25)]"><Sparkles size={26} /></div><div className={cn('min-w-0', mobileOpen ? 'block' : 'hidden')}><div className="truncate text-sm font-semibold tracking-tight">Hermes Studio</div><div className="text-[11px] text-muted-foreground">Agent workspace</div></div>{mobileOpen && onClose && <Button ref={closeButton} variant="ghost" size="icon" className="ml-auto lg:hidden" onClick={onClose} aria-label="Close navigation"><X size={16} /></Button>}</div>
    <nav data-testid="primary-navigation" className={cn('mt-5 flex min-h-0 flex-1 flex-col gap-1', mobileOpen ? 'w-full' : 'w-14')} aria-label="Main navigation">{items.filter(item => visible.has(item.label)).map(({ label, icon: Icon }) => <Tooltip key={label} label={label}><Button variant="ghost" onClick={() => { onNavigate(label.toLowerCase()); onClose?.() }} aria-label={label} aria-current={currentView === label.toLowerCase() ? 'page' : undefined} className={cn('h-14', mobileOpen ? 'w-full justify-start' : 'w-14 justify-center px-0', currentView === label.toLowerCase() ? 'bg-accent text-accent-foreground' : 'text-muted-foreground')}><Icon aria-hidden="true" size={24} /><span className={cn(mobileOpen ? 'inline' : 'hidden')}>{label}</span></Button></Tooltip>)}</nav>
    <Tooltip label="Settings"><Button variant="ghost" size="icon" className={cn('size-14 shrink-0', mobileOpen ? 'w-full justify-start px-4' : '')} onClick={() => { onNavigate('settings'); onClose?.() }} aria-label="Settings"><Settings2 size={24} /><span className={cn(mobileOpen ? 'inline' : 'hidden')}>Settings</span></Button></Tooltip>
