    ...
    <aside className={cn('h-screen w-[264px] shrink-0 flex-col border-r bg-card/55 p-3 backdrop-blur-xl', mobileOpen ? 'fixed inset-y-0 left-0 z-50 flex shadow-2xl' : 'hidden lg:flex')}>
      ...
      <nav className="mt-5 space-y-1" aria-label="Main navigation">
        {navigation.map(({ label, icon: Icon }) => (
          <Button key={label} variant="ghost" onClick={() => { onNavigate(label.toLowerCase()); onClose?.() }} aria-current={currentView === label.toLowerCase() ? 'page' : undefined} className={cn('h-9 w-full justify-start', currentView === label.toLowerCase() ? 'bg-accent text-accent-foreground' : 'text-muted-foreground')}>
            <Icon size={16} /><span>{label}</span>
          </Button>
        ))}
      </nav>

      {currentView === 'chat' && <div className="mt-6 flex items-center gap-2 px-2 text-[11px] font-medium uppercase tracking-[0.16em] text-muted-foreground"><label htmlFor="session-search">Recent sessions</label><Search size={13} className="ml-auto" /></div>}
      {currentView === 'chat' && <Input id="session-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search sessions" className="mt-2 h-8 bg-background/50 text-xs" />}
      {currentView === 'chat' && <>
