...
export function ContextRail({ title, subtitle, open, onToggle, action, children, className }: Props) {
  if (!open) return null
  return <aside data-testid="context-rail" className={cn('context-rail context-rail--shared hidden h-full min-h-0 shrink-0 flex-col border-r bg-card/25 p-3 lg:flex', className)} aria-label={`Context ${title} navigation`}>
