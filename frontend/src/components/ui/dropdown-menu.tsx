...
export function DropdownMenu({ label, items }: { label: string; items: MenuItem[] }) {
  ...
  return <div ref={rootRef} className="relative shrink-0">
    <Button ref={triggerRef} type="button" variant="ghost" size="icon" className="size-8" aria-label={label} aria-expanded={open} aria-haspopup="menu" onClick={() => setOpen(value => !value)}><MoreVertical size={16} /></Button>
