...
export function useWorkspace() {
  const [open, setOpenState] = useState(() => window.localStorage.getItem('hermes-workspace-open') === 'true'); const [path, setPath] = useState('.'); const [entries, setEntries] = useState<WorkspaceEntry[]>([]); const [preview, setPreview] = useState<WorkspacePreview | null>(null); const [git, setGit] = useState<GitStatus | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState('')
  const setOpen = useCallback((value: boolean | ((current: boolean) => boolean)) => { setOpenState(current => { const next = typeof value === 'function' ? value(current) : value; window.localStorage.setItem('hermes-workspace-open', String(next)); return next }) }, [])
