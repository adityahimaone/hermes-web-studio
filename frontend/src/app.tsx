  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main id="main-content" className="flex min-w-0 flex-1 flex-col" aria-label="Hermes Studio workspace">
        ...
        {view === 'chat' ? <div className="flex min-h-0 flex-1">{chatSessionsOpen && <SessionRail sessions={chat.sessions} activeSessionId={chat.activeSessionId} onSelectSession={chat.selectSession} onSearch={chat.searchSessions} onRename={chat.rename} onPin={chat.pin} onArchive={chat.archive} onDelete={chat.remove} onDuplicate={chat.duplicate} onNewChat={() => { chat.reset(); setView('chat'); setChatSessionsOpen(true) }} loading={chat.sessionLoading} error={chat.sessionError} onToggle={() => setChatSessionsOpen(false)} />}<div className="flex min-w-0 flex-1 flex-col"><div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto"><MessageList messages={chat.messages} stream={chat.streamState} onEdit={chat.edit} onRetry={chat.retry} onApproval={chat.approve} /></div><Composer onSend={chat.send} onCommand={command => { if (command === '/clear') chat.reset(); else if (command === '/help') chat.setDraft('Try /clear to reset this conversation, or write a message for Hermes.') }} onCancel={chat.cancel} onRemoveQueued={chat.removeQueued} isStreaming={chat.isStreaming} draft={chat.draft} onDraftChange={chat.setDraft} queuedMessages={chat.queuedMessages} contextUsage={chat.streamState.usage} workspacePath={workspace.path} onWorkspaceOpen={() => workspace.setOpen(true)} /></div></div> : <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto"><ControlCenter view={view} onCustomizeNavigation={() => setCustomizeNavigationOpen(true)} /></div>}
